...
function BulkRunDetail({ run, members, gid, onBack, onRunUpdated }) {
  ...
  const loadItems = useCallback(async () => {
    setLoading(true)
    try {
      const res = await api.get(`/pantry/${gid}/bulk-buys/${run.id}/items`)
      setItems(res.data)
  ...
  const addItem = async form => {
    await api.post(`/pantry/${gid}/bulk-buys/${run.id}/items`, form)
    await loadItems()
  }
  const deleteItem = async id => {
    if (!confirm('Remove item?')) return
    await api.delete(`/pantry/${gid}/bulk-buys/${run.id}/items/${id}`)
    await loadItems()
  }
  const saveActualCost = async item => {
    await api.put(`/pantry/${gid}/bulk-buys/${run.id}/items/${item.id}`, { actual_cost: costVal === '' ? null : parseFloat(costVal) })
    setEditingCost(null)
    await loadItems()
  }
  const fetchSettlement = async () => {
    const res = await api.get(`/pantry/${gid}/bulk-buys/${run.id}/settlement`)
    setSettlement(res.data)
    setShowSettlement(true)
  }
  const saveRunEdits = async () => {
    const res = await api.put(`/pantry/${gid}/bulk-buys/${run.id}`, runForm)
...
export default function Pantry({ locationFilter, defaultTab }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [iRes, vRes, rRes, mRes] = await Promise.all([
        api.get(`/pantry/${gid}/inventory`),
        api.get(`/pantry/${gid}/vacuum-log`),
        api.get(`/pantry/${gid}/bulk-buys`),
  ...
  const addInv = async form => {
    setAddingInv(true)
    try { await api.post(`/pantry/${gid}/inventory`, form); await load() }
    finally { setAddingInv(false) }
  }
  const deleteInv = async id => {
    if (!confirm('Remove item?')) return
    await api.delete(`/pantry/${gid}/inventory/${id}`)
    setInv(v => v.filter(i => i.id !== id))
  }
  const addVs = async form => {
    setAddingVs(true)
    try { await api.post(`/pantry/${gid}/vacuum-log`, form); await load() }
    finally { setAddingVs(false) }
  }
  const deleteVs = async id => {
    if (!confirm('Remove log entry?')) return
    await api.delete(`/pantry/${gid}/vacuum-log/${id}`)
    setVsLog(v => v.filter(i => i.id !== id))
  }
  const addRun = async form => {
    const res = await api.post(`/pantry/${gid}/bulk-buys`, form)
    await load()
    return res.data
  }
  const deleteRun = async run => {
    if (!confirm(`Delete "${run.name}"?`)) return
    await api.delete(`/pantry/${gid}/bulk-buys/${run.id}`)
    await load()
  }
